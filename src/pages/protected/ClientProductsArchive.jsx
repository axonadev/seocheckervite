...
yearOptions.forEach((y) =>
  monthOptions.forEach((m) => monthYearOptions.push(`${m} ${y}`))
);
...
const formatDateToMonthYear = (dateString) => {
  ...
  try {
    ...
    if (
      existingFormatParts.length === 2 &&
      monthOptions.includes(existingFormatParts[0].toUpperCase())
    ) {
      ...
      <Select
        ...
        onChange={(e) =>
          params.api.setEditCellValue(
            { id: params.id, field: "seo", value: e.target.value },
            e
          )
        }
        sx={{ minWidth: 80 }}
  ...
  },
  {
    ...
    renderEditCell: (params) => (
      <Select
        ...
        onChange={(e) =>
          params.api.setEditCellValue(
            { id: params.id, field: "multilanding", value: e.target.value },
            e
          )
        }
        sx={{ minWidth: 80 }}
  ...
  },
  {
    ...
    renderEditCell: (params) => (
      <TextField
        ...
        onChange={(e) =>
          params.api.setEditCellValue(
            { id: params.id, field: "news", value: e.target.value },
            e
          )
        }
        sx={{ minWidth: 80 }}
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Select
            ...
            onChange={(e) => {
              ...
              params.api.setEditCellValue(
                { id: params.id, field: "inizioContratto", value: newValue },
                e
              );
            }}
            sx={{ minWidth: 100 }}
      ...
      if (progettiResponse?.Itemset?.progettiserp) {
        const clienti = progettiResponse.Itemset.progettiserp.map((item) => {
          ...
          return {
            ...
            seo: seoProd
              ? seoProd.ProgettiSerpProdotti_Valore === true ||
                String(seoProd.ProgettiSerpProdotti_Valore).toUpperCase() ===
                  "SI" ||
                String(seoProd.ProgettiSerpProdotti_Valore) === "1"
              : false,
            idseo: seoProd ? seoProd.IDOBJ : undefined,
            // Map to boolean true/false
            multilanding: multilandingProd
              ? multilandingProd.ProgettiSerpProdotti_Valore === true ||
                String(
                  multilandingProd.ProgettiSerpProdotti_Valore
                ).toUpperCase() === "SI" ||
                String(multilandingProd.ProgettiSerpProdotti_Valore) === "1"
              : false,
            idmultilanding: multilandingProd
              ? multilandingProd.IDOBJ
              : undefined,
            news: item.ProgettiSerp_News || "",
            // Format the date here before setting state
            inizioContratto: formatDateToMonthYear(
              item.ProgettiSerp_InizioContratto || ""
            ),
          };
        });
        setClients(clienti);
      ...
      if (field === "seo" || field === "multilanding") {
        ...
        if (!existingRecordId) {
          ...
          if (leggiResponse?.Itemset?.ProgettiSerpProdotti?.[0]?.IDOBJ) {
            existingRecordId =
              leggiResponse.Itemset.ProgettiSerpProdotti[0].IDOBJ;
            console.log(
              `Found existing record ID for ${field}: ${existingRecordId}`
            );
          }
        }
        ...
        const scrittoResponse = await Scrivi(
          SERVERAPI,
          token,
          idobj,
          DB,
  ...
  const processRowUpdate = async (newRow, oldRow) => {
    ...
    try {
      ...
      if (newRow.seo !== oldRow.seo) {
        promises.push(
          saveClientField(newRow.id, "seo", newRow.seo, newRow.idseo).then(
            (newId) => {
              if (newId) idUpdates.idseo = newId;
            }
      ...
      if (newRow.multilanding !== oldRow.multilanding) {
        promises.push(
          saveClientField(
            ...
            "multilanding",
            newRow.multilanding,
            newRow.idmultilanding
          ).then((newId) => {
            if (newId) idUpdates.idmultilanding = newId;
          }) // Store new ID if returned
        );
      }
      ...
      if (newRow.inizioContratto !== oldRow.inizioContratto) {
        promises.push(
          saveClientField(newRow.id, "inizioContratto", newRow.inizioContratto)
        ); // No ID expected back
      }
